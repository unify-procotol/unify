...
- **Automatic Operation Logging**: Logs all CRUD operations with timing information
- **Custom Logger Support**: Use your own logging implementation instead of console.log
- **Rich Context Information**: Includes operation type, arguments, results, and execution time
- **Error Tracking**: Automatically logs operation failures with error details
...
const app = Unify.init({
  middleware: [winstonLoggingMiddleware],
});
```
