...
export default PagesUnify.init({
  plugins: [WalletPlugin]
});
```
...
```typescript
class EVMAdapter implements DataSourceAdapter<WalletEntity> {
...
```typescript
class SolanaAdapter implements DataSourceAdapter<WalletEntity> {
...
const WalletPlugin: Plugin = {
  ...
  adapters: [
    { source: "evm", entityName: "wallet", adapter: new EVMAdapter() },
    { source: "solana", entityName: "wallet", adapter: new SolanaAdapter() }
  ]
};
```
...
const wallets = await repo<WalletEntity>("wallet", "solana").findMany({
  where: { 
...
class DatabaseAdapter implements DataSourceAdapter<UserEntity> {
...
const UserPlugin: Plugin = {
  entities: [UserEntity],
  adapters: [
    { source: "database", entityName: "user", adapter: new DatabaseAdapter() }
  ]
};
```
