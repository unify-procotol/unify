---
title: API Reference
description: Complete API reference for Unify packages including @unify/core, @unify/server, and @unify/client.
---
...
```typescript
interface FindManyArgs<T extends BaseEntity> {
  limit?: number;
  offset?: number;
...
```typescript
interface FindOneArgs<T extends BaseEntity> {
  where: Partial<T>;
  select?: Array<keyof T>;
}
```
...
```typescript
interface CreationArgs<T extends BaseEntity> {
  data: Partial<T>;
}
```
...
```typescript
interface UpdateArgs<T extends BaseEntity> {
  where: Partial<T>;
  data: Partial<T>;
}
```
...
```typescript
interface DeletionArgs<T extends BaseEntity> {
  where: Partial<T>;
}
```
...
```typescript
interface DataSourceAdapter<T extends BaseEntity> {
...
```typescript
function registerAdapter<T extends BaseEntity>(
  name: string,
...
```typescript
function getAdapter<T extends BaseEntity>(
  source: string
): DataSourceAdapter<T>
```
...
```typescript
function parseQueryParams<T extends BaseEntity>(c: Context): any
```
...
```typescript
import { UnifyClient, Repo } from "@unify/client";
import { WalletEntity } from "@unify/uniweb3";
...
```typescript
import type { DataSourceAdapter, BaseEntity } from "@unify/core";
