...
joinRepo<PostEntity, UserEntity>("post", "post", {
  ...
  const data = await repo<UserEntity>("user", "user").findMany({
    ...
    include: {
      posts: (userList) => {
        ...
        // Use joinRepo for optimized relation mapping
...
- 🎯 **Flexible Relations**: Define relation logic at query time with callbacks
- 🚀 **Type Safe**: Full TypeScript support with automatic type inference
- 📊 **Optimized Joins**: Use `joinRepo` for efficient relation mappings
- 🔧 **Simple Setup**: Minimal configuration required
- 💪 **Powerful**: Access to full entity data in relation callbacks
- ⚡ **Performance**: Client-side parallel relation loading
