...
import { generateSchemas, Plugin, WhereCondition } from "@unilab/core";
...
export interface UnifyToolConfig {
  id: string;
  description: string;
  plugin: Plugin;
...
/**
 * Creates a universal Unify tool that can automatically parse plugin information
 * and generate repository calls dynamically
 */
export function createUnifyTool(config: UnifyToolConfig) {
  const {
    id,
    description,
    plugin,
...
export class UnifyToolBuilder {
   ...
   */
  createTool(config: Omit<UnifyToolConfig, "plugin" | "schemas">) {
    return createUnifyTool({
      ...config,
      plugin: this.plugin,
...
/**
 * Enhanced UnifyToolBuilder with smart capabilities
 */
export class EnhancedUnifyToolBuilder extends UnifyToolBuilder {
  private smartTool: SmartUnifyTool;

  constructor(plugin: Plugin) {
    super(plugin);
    this.smartTool = new SmartUnifyTool(plugin);
  }
...
export function createUniversalToolFromPlugin(plugin: Plugin) {
  const builder = new EnhancedUnifyToolBuilder(plugin);
